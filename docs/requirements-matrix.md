...
1. 查闲置桩（`pile_list`）→ 2. 预约（`reserve`，桩 idle→busy，写 `charge_order` reserved）→ 3. 开始充电消息 `start_charge`（订单→charging，回 `start_time,power_kw,unit_price`）→ 4. 结束充电出账 `finish_charge`（服务端按功率×时长算费，订单→pending_payment，释放电桩，不扣款）→ 5. 确认支付 `pay_charge`（扣款、写流水、订单→settled；余额不足 code=7 且保持待支付）。闭环完成的标志：状态、费用、余额、订单四项都结束。旧 `settle` 已停用。
...
| 17 |  |  | 用户可从本地选择图片并更换头像。 | 通过 QFileDialog 选择 jpg/png，校验大小与格式后发送 {type:'update_profile', token, data:{avatar}}（**data 不含 user_id**，avatar 为 `avatars/user_<id>.png` 路径字符串）。服务器用 token 反查 userId 再写库；成功后才落盘并刷新头像，失败保留旧图；token 无效返回 code=9。 | 马晓钰 | 2026-09-04 | ○ |  |
...
| 22 |  |  | 服务器原子化锁定空闲电桩并创建预约订单。 | 分发层用 token 得到 sess.userId 后开启事务：条件更新 pile 为 busy/reserved，插入 charge_order（user_id 取自会话而非报文）。同时检查该用户无未完成订单（reserved/charging/pending_payment，与 unfinished_order 对齐）；失败整单回滚。客户端伪造 data.user_id 无效。 | 朱雅琪 | 2026-09-06 | ○ |  |
...
| 30 |  | 销售业绩 | 管理端可切换查看近 7 日和近 30 日营收数据。 | 切换 7/30 日后发送 {type:'admin_revenue_trend', token, data:{days:7\|30}}（须管理员 token；须 `admin_` 前缀才能过管理员鉴权门）。横轴按自然日补 0；切换模块返回时保持原时间维度。 | 牛昀轶 | 2026-09-05 | ○ |  |
...
| 75 |  | 对应NO.17 用户-改头像 | 处理资料消息 update_profile（avatar） | 【对应用户端NO.17】【消息】type=update_profile。【客户端发】顶层 token；data.avatar（路径字符串，≤255）；data 不含当前用户 user_id；可只传 avatar。【服务端收】validate token 且 role=user；userId 取自会话；非法类型/超长 code=2。【库操作】UPDATE user SET avatar=? WHERE id=会话userId。【服务端回】code=0 时 data.avatar；code=9 会话无效。用户端选图发包见 NO.17。 | 翟梓涵 | 2026-09-06 | ○ |  |
...
| 77 |  | 对应NO.19 用户-充值 | 处理充值消息 recharge（独立业务） | 【对应用户端NO.19】【消息】type=recharge。【客户端发】顶层 token；data.amount(正数,元)；不传 user_id。【服务端收】validate role=user；amount≤0 则 code=2。【库操作】`runInTransaction`：SELECT user.balance FOR UPDATE；INSERT wallet_transactions(type=recharge,amount,balance_before,balance_after,transaction_no)；UPDATE user.balance。【服务端回】code=0 时 data.balance=新余额。【客户端展示】刷新钱包。 | 翟梓涵 | 2026-09-05 | ○ |  |
| 78 |  | 对应NO.20 充电-未完成订单 | 处理未完成订单查询 unfinished_order | 【对应用户端NO.20】【消息】type=unfinished_order。【客户端发】顶层 token；data 可空。【服务端收】validate；userId 仅来自会话。【库操作】SELECT charge_order 最新一条 WHERE user_id=会话 AND status IN (reserved,charging,pending_payment)。【服务端回】code=0 时 data.order 有则含 order_no,pile_id,status,power_kw,unit_price,duration_seconds,kwh,amount 等，无则空。charging 按时长现算；pending_payment 用已出账单。【客户端展示】有单则进充电/结算页。 | 翟梓涵 | 2026-09-05 | ○ |  |
...
| 80 |  | 对应NO.22 充电-预约落库 | 预约处理（锁桩+写订单，闭环第 2 步） | 【对应用户端NO.22】【消息】type=reserve。【客户端发】token + data.pile_id。【服务端收】validate role=user；pile_id≤0 code=2；伪造 data.user_id 无效。【库操作】`runInTransaction`：检查 user.status≠frozen；无未完成订单（status IN reserved/charging/pending_payment，与 unfinished_order 对齐）否则 code=8；SELECT pile+station FOR UPDATE；pile.status 必须 idle；INSERT charge_order(order_no,user_id=会话,station_id,pile_id,status=reserved,unit_price=station.price,reserve_time)；UPDATE pile.status=busy。【服务端回】code=0 时 data.order_no；失败整单回滚。 | 朱雅琪 | 2026-09-06 | ○ |  |
| 81 |  | 对应NO.23 充电-开始充电 | 处理开始充电消息 start_charge（闭环第 3 步：改桩状态） | 【对应用户端NO.23】【消息】type=start_charge。【客户端发】顶层 token；data.order_no。【服务端收】validate；解析 order_no；身份用会话 userId。【库操作】校验 charge_order 属于该用户且 status=reserved；桩非 fault；UPDATE 订单 status=charging、start_time=NOW()；桩保持 busy。【服务端回】code=0 时 data.start_time、power_kw、unit_price；非本人订单 code=4。 | 朱雅琪 | 2026-09-06 | ○ |  |
...
| 100 |  | 对应NO.42 管理-站内桩明细 | 按 station_id 查桩（管理员角色） | 【对应管理端NO.42】【消息】复用 `admin_pile_list`（不新增 type）。【客户端发】管理员 token；`data.station_id`（详情建议 `page_size=50`）。【服务端收】validate admin；`station_id` 非法 code=2；站不存在/已删 code=4。【库操作】读该站 pile.code,type,power_kw,status，以及当前 `reserved/charging` 订单号 `order_no`、`last_online_at`。【服务端回】桩数组（分页字段不变；`stats` 仍为全局）。【客户端展示】10 秒刷新见 NO.42。 | 翟梓涵 | 2026-09-06 | ○ |  |
