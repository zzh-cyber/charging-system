...
| 13 |  | 用户信息维护 | 用户输入 11 位手机号即可发起免密登录。 | 登录页使用 QLineEdit 限制仅输入数字、最大长度 11，并用正则 ^1\\d{10}$ 做提交校验；不合法时在输入框下显示原因。合法后发送 {type:'login', data:{phone, register:false}}（**登录请求不带 token**），提交期间禁用按钮。未注册返回 code=4 且 data.can_register=true，界面显示「注册」按钮。手机号仅用于本实训模拟登录，不发送验证码；日志对中间四位脱敏。 | 马晓钰 | 2026-09-02 | ○ |  |
...
| 83 |  | 对应NO.25 充电-结束计费 | 结束充电后的费用计算（闭环第 4 步前半：先处理桩与费用） | 【对应用户端NO.25】【消息】type=settle（结束充电与结算同一报文）。【客户端发】token；data.order_no、data.kwh。【服务端收】validate；校验订单属于会话用户且 status=charging。【库操作】与 NO.84 同一次 `runInTransaction`：amount=kwh×charge_order.unit_price（下单固化单价）；写 kwh、amount、end_time、duration_seconds。【服务端回】data 含 amount 供用户核对（老师所说可先回费用再扣款）。【说明】当前实现算费与扣款在同一次 settle 完成。 | 翟梓涵 | 2026-09-07 | △ |  |
| 84 |  | 对应NO.26 充电-扣款释放订单 | 结算处理+订单记录（闭环第 4/5 步后半：余额与订单） | 【对应用户端NO.26】【消息】type=settle。【客户端发】同 NO.83。【服务端收】同 NO.83。【库操作】`runInTransaction`：读 user.balance FOR UPDATE；足则扣款、INSERT wallet_transactions(type=charge_pay)、订单 status=settled、pay_request_id 幂等；不足则当前返回 code=7 且整单回滚（pending_payment 落库仍待补）；UPDATE pile.status=idle 并累加 total_count、total_hours。【服务端回】code=0 时 data.amount、data.balance。【闭环完成】桩状态、费用、余额、charge_order 订单记录（用户/站桩/时间/电量/金额）四项均结束，禁止凭他人 order_no 操作。 | 朱雅琪 | 2026-09-07 | △ |  |
