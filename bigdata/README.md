...
python3 bigdata/gen_ods.py --orders \
    ...
    --seed 20260914 \               # 换种子换一份数据（但同种子不保证同数据，见下）
...
**可复现是【有条件的】**：同一组
`(--seed --end-date --sim-now --days --rows --dirty-ratio)`
下输出逐字节相同，**但前提是演示库的 `station` / `pile` / `user` 三张表
也和上次一模一样**。解析后的参数会原样写进 `dq_expected.json` 备查。

> ⚠️ **换台机器、或隔几天再跑，都复现不出原文件。** 两个原因：
>
> 1. **维表是从库里 `SELECT` 现读的**（`gen_ods.py:load_dimensions`），不是从
>    仓库读的。只要有人注册一个用户、或用管理员端建一个站/桩，`usable` 列表
>    就变了。实测：只少一个站，20000 行内容**全变** —— 所有抽取共用一个
...
| 字段 | 用途 |
|---|---|
| `params` | 本次生成的种子与参数。⚠️ 照抄这些**不足以**复现，还得有同一个演示库快照 —— 见上面「可复现是有条件的」 |
...
| `file_sha256` | **三份产物的指纹**。这份数据在别的机器上复现不出来（见上），所以「你手里的是不是验收用的那一份」只能靠它对，不能靠参数对 |

> 对指纹（拿到 HDFS 上的文件后第一件事）：
>
> ```bash
...
7. **在 Spark 里按矩阵的清洗规则清洗**，核对 `dwd_rows` 与 `dwd_kwh_total`
...
| 清洗丢弃明细 + 站点单量 | NO.114 矩阵清洗规则各丢多少、总量对不对 |
...
1. **`dq_tag` 在 DWD 之后必须 `drop` 掉**，而且**不许拿它当清洗依据**。
   DWD 要按矩阵的清洗规则真清洗（丢弃 `start_time` 为空 / `kwh` 为空 /
   `kwh < 0` / 时间颠倒 / 孤儿 / 状态矛盾；重复单号留 `updated_at` 最新一条），
