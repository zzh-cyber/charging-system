    ...
    , m_net(new NetClient(this))
{
    ...
    m_userEdit->setPlaceholderText(QStringLiteral("管理员账号"));
    ...
    connect(m_loginBtn, &QPushButton::clicked, this, &AdminLoginWindow::onLoginClicked);
}
...
void AdminLoginWindow::onLoginClicked()
{
    ...
    const QJsonObject resp =
        m_net->request(Protocol::makeRequest(Protocol::MsgType::AdminLogin, data));

    if (resp.value("code").toInt() != Protocol::Ok) {
        QMessageBox::warning(this, QStringLiteral("登录失败"), resp.value("msg").toString());
        return;
    }
    ...
    // TODO(管理端负责人)：进入后台主界面（数据总览 / 电桩 / 电站 / 用户 / 订单）
}
